...
//  Bidders : 1000 (created in setup)
//  Auctions: 100  (created in setup)
...
const SOCKET_URL      = 'ws://49.12.201.167/socket.io/?EIO=4&transport=websocket';
...
// ── Token refresh — re-login if token expired (401) ──────────
function refreshBidderToken(bidder) {
  const res  = http.post(
    `${BASE_URL}/user/login`,
...
export function setup() {
  ...
  // Step 2: Create 1000 bidder accounts
  ...
  for (let i = 1; i <= 1000; i++) {
    ...
    if (res.status === 200 && body && body.success) {
      // Pre-login to get token during setup — avoids login during test
      const plr = http.post(
        `${BASE_URL}/user/login`,
        JSON.stringify({ email, password: BIDDER_PASSWORD, type: 'bidder' }),
        { headers: { 'Content-Type': 'application/json' } }
      );
      const plb = safeJSON(plr);
      bidders.push({ id: body.data.id, email, password: BIDDER_PASSWORD, token: plb ? plb.token : null });
      ...
      if (lb && lb.data && lb.data.id) {
        bidders.push({ id: lb.data.id, email, password: BIDDER_PASSWORD, token: lb.token });
...
export function bidderFlow(data) {
  ...
  // ── FIX 1: Use pre-stored token — skip login during test ──
  let token  = bidder.token;
  let userId = bidder.id;

  // Fallback: login only if token missing (first run edge case)
  if (!token) {
  ...
  // List auctions — refresh token if expired (401)
  const aStart   = Date.now();
  let   auctionRes = http.get(`${BASE_URL}/auction/auctions`, { headers: authHdr });
  if (auctionRes.status === 401) {
    token = refreshBidderToken(bidder);
    if (!token) { errorRate.add(1); sleep(2); return; }
    authHdr.Authorization = `Bearer ${token}`;
    auctionRes = http.get(`${BASE_URL}/auction/auctions`, { headers: authHdr });
  }
  check(auctionRes, { 'auction list 200': r => r.status === 200 });
  ...
  // ── FIX 2: Single WebSocket connection only ───────────────
  ws.connect(
      SOCKET_URL,
      {},
      function (socket) {

        // ── FIX 3: Count bids only on server confirmation ──
        socket.on('message', (msg) => {
          if (msg.includes('bid:success') || msg.includes('bid:accepted')) {
            bidSuccess.add(1);
          } else if (msg.includes('bid:error') || msg.includes('bid:failed')) {
            bidFail.add(1);
          }
        });

        socket.on('error', (e) => {
          console.error(`Socket error VU${__VU}: ${e}`);
        });
        ...
        socket.on('open', () => {
          ...
          socket.send(`42["join",{"id":${userId}}]`);

          sleep(0.5);

          // ── Join auction room ──────────────────────────────
          socketEmit(socket, 'auction:join', { auction_id: auctionId });
          ...
          for (let b = 0; b < bidCount; b++) {
            ...
            socketEmit(socket, 'auction:bid', { auction_id: auctionId, user_id: userId });
            bidDuration.add(Date.now() - bStart);
            // ── bid counted in message handler above, not here ──
            sleep(Math.random() * 2 + 1);
          }

          // ── Leave room ─────────────────────────────────────
          socketEmit(socket, 'auction:leave', { auction_id: auctionId });
          sleep(0.5);
...
export function userFlow(data) {
  ...
  // ── FIX 1: Use pre-stored seller token — skip login during test ──
  let token = data.token;

  // Fallback: login only if token missing
  if (!token) {
    const loginStart = Date.now();
    const loginRes   = http.post(
      `${BASE_URL}/user/login`,
  ...
  // List auctions — refresh token if expired (401)
  const aStart     = Date.now();
  let   auctionRes = http.get(`${BASE_URL}/auction/auctions`, { headers: jsonHdr });
  if (auctionRes.status === 401) {
    token = refreshUserToken();
    if (!token) { errorRate.add(1); sleep(2); return; }
    jsonHdr.Authorization = `Bearer ${token}`;
    auctionRes = http.get(`${BASE_URL}/auction/auctions`, { headers: jsonHdr });
  }
  check(auctionRes, { 'user: list 200': r => r.status === 200 });
  auctionDuration.add(Date.now() - aStart);
  sleep(Math.random() + 1);
